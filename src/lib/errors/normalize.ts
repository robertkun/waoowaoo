...
function inferCodeFromMessage(message: string): UnifiedErrorCode | null {
  ...
  if (containsAny(message, ['sensitive', 'unsafe', 'safety', 'blocked', 'prohibited', 'policy_violation', 'moderation', 'harm', '敏感', '违规', '不当', '审核未通过', 'content verification failed', '内容审核']) && !containsAny(message, ['case-sensitive', 'case sensitive'])) return 'SENSITIVE_CONTENT'
